...
% Classification: are drawn carefully on the grid produced by the function if guarding
% There as the matrix T.

function mpaper(varargin)

warning off MATLAB:divideByZero

% In this practical work this function is simply used
% To generate inputs from a grid where the user draws with
% Mouse, a digit (0-9). The number in the cell is divided into a
% 16x16 grid and each grid is transformed into a 1 or a 0 as
% Or not completed by that cell line drawn.
%
% You can call a script with a variable number of arguments (varargin, variable arguments input) and run
% Directly from the command line by typing just mpaper. Pops up
% A grid 5x10. Drawing up a digit in each cell.
% The result of "scanning" of the grid is stored in the structure
% Data.X. Each column line 256 corresponds to a square.
%
% This file was adapted by J. Henriques to the chair Computer Adaptive
%. See below original authorship.

%Summing up:
% 1 runs mpaper (without rating, commenting feval line # 192 this file)

% 2 draw up characters in sufficient numbers, one in each box
% Presented to construct the training data.

% 2 uses the matrix of P inputs thus created for our problem,
% Activating the lines 178-181 and deactivating the line 200 and 201.

% 3rd They draw up, much like the perfect character in the grid Mpaper
%, which will be stored as the matrix T, deactivating the line 181 and
% Activating the line 199 of this ficheiro.Alternativamente you use the Perfect Arial.mat file. 4

% To graphed each entry in a grid, it uses the graphical function
% (X, Y, Z), which allows you to view one, two, or three inputs.
